...
It is authored here, in `og-image.typ`, so you can regenerate the card from source.
...
Two rows sit on one grid, the second smaller and dimmer. That says the substitution is systematic, without turning the card into a table.
...
It is kept faint enough to be texture. Any louder, and the eye reads it instead of the substitution. The card would start to look like it is showing a real document, rather than standing for one.
...
The template's page is 1200pt by 630pt. So `--ppi 144` exports at 2400x1260, and the resize brings it back to the design size. That is what Open Graph consumers expect, and what `og:image:width` and `og:image:height` report.
