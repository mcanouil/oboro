...
It carries the pinned Rust toolchain, Tesseract and the OCR libraries. Your machine only needs Docker. `cargo build --release --features "ner,ocr"` builds every feature inside it, with nothing extra to install.
...
The lint configuration denies all of `clippy::all`, and warns on `pedantic`. CI treats warnings as errors.
...
It runs over every readable format. So you cannot add a converter without covering it.
...
They assert recall. That is the guarantee this layer offers. They do not assert precision, which it cannot promise.
...
Returning part of a document is the worst outcome available: the result looks sanitised without ever having been read.
