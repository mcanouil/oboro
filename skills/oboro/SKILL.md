...
Oboro replaced it on the way to you. The real value is in the vault.
...
An invented address written into a file is a real error. A placeholder written into a file is restored correctly.

Never invent a placeholder that was not in the text you were given.
Oboro leaves a placeholder the vault never issued in place, and reports it to the user. So it lands in the file as literal `[[TAG_n]]` text.
...
If a user is about to paste sensitive text, suggest they let a tool read the file instead. That way the hook covers it.
...
In a tool with no hooks, these commands are the primary path, not a fallback.
