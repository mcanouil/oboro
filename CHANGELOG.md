...
- feat: Read `.odt` OpenDocument text files, including the headers and footers that live in the document's styles rather than its body, and the annotations anchored inside a paragraph, so a letterhead or a contact line is not silently missing from what is read. (#81)
...
- fix: Read `.odt` annotation metadata by position rather than by naming each element that carries it, so a piece of metadata this reader had not been told about, such as `meta:date-string`, no longer glues onto the paragraph next to it with no separator; a commenter's name concatenated like that no longer has a word boundary on the side that touches, so it stopped matching a denylisted name and reached output unredacted. (#81)
