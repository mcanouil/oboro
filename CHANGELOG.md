...
- feat: Read `.pptx` presentations, taking the text of every slide in presentation order along with its speaker notes and comments, so a deck reaches the detectors instead of being refused; slide layouts and masters are left out, since their text is template prompt material rather than anything the author wrote. (#81)
